...
namespace eShopSolution.Application.Category.ProductServices
{
    public class ProductService : IProductService
    {
        ...
        public Task<int> AddProductImageAsync(CreateProductImageRequest createNewRequest)
        {
        ...
        public async Task<int> CreateProductAsync(CreateProductRequest createNewRequest)
        {
        ...
        public async Task<PagedResult<ProductViewModel>> GetAllProductPagingByCategoriesAsync(GetProductsPagingByCategoriesRequest request)
        {
        ...
        public async Task<PagedResult<ProductViewModel>> GetAllProductPagingByCategoryAsync(GetProductsPagingByCategoryRequest request)
        {
            var query = from p in context.Products
                        join pt in context.ProductTranslations on p.Id equals pt.ProductId
                        join pic in context.ProductInCategories on p.Id equals pic.ProductId
        ...
        public async Task<int> RemoveProductImageAsync(int imageId)
        {
            var image = await context.ProductImages.FindAsync(imageId);
            if (image == null) throw new EShopException($"Cannot find image with id {imageId}");
        ...
        public async Task<int> UpdateProductAsync(UpdateProductRequest updateRequest)
        {
            var product = await context.Products.FindAsync(updateRequest.ProductId);
            var productTranslation = await context.ProductTranslations.FirstOrDefaultAsync(x => x.ProductId == updateRequest.ProductId && x.LanguageId == updateRequest.LanguageId);
            if (product == null || productTranslation==null) throw new EShopException($"Cannot find product with id: {updateRequest.ProductId}");
            ...
            {
                var imageThumbnail = await context.ProductImages.FirstOrDefaultAsync(x => x.IsDefault == true && x.ProductId == updateRequest.ProductId);
        ...
        public async Task<int> UpdateProductImageAsync(int imageId, UpdateProductImageRequest request)
        {
            var image = await context.ProductImages.FindAsync(imageId);
            if (image == null) throw new EShopException($"Cannot find image with id:{imageId}");
